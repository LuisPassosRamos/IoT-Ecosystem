...
Publishes to MQTT topic: sensors/humidity/hum-001
"""
...
# Sensor parameters
NORMAL_RANGE = (30.0, 80.0)  # Normal humidity range in %
ANOMALY_THRESHOLD = 15.0  # Humidity jump threshold for anomaly
UPDATE_INTERVAL = 3  # seconds

class HumiditySensor:
    def __init__(self):
        self.client = mqtt.Client(client_id=f"sensor-{SENSOR_ID}")
        self.last_value = 55.0  # Start with moderate humidity
        self.setup_mqtt()
    ...
    def generate_reading(self):
        ...
        # 93% normal readings, 7% anomalous
        if random.random() < 0.07:
            # Generate anomalous reading
            if random.random() < 0.6:
                # Out of range (too dry or too humid)
                value = random.choice([
                    random.uniform(5.0, 25.0),   # Too dry
                    random.uniform(85.0, 100.0)  # Too humid
                ])
            else:
                # Sudden jump
